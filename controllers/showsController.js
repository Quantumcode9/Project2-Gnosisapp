...
const LATEST_TV_SHOWS_URL = '/trending/tv/day?language=en-US&page=2';
const API_BASE_URL = 'https://api.themoviedb.org/3';
const POPULAR_TV_SHOWS_URL = '/tv/popular?language=en-US&page=2';
const HEADERS = {
...
router.get('/pages/genre/:genreId', async (req, res) => {
  ...
  const url = `${API_BASE_URL}/discover/tv?include_adult=false&include_null_first_air_dates=false&language=en-US&page=2&sort_by=popularity.desc&with_genres=${genreId}`;
  ...
  try {
    ...
    res.render('pages/genre', { genre: genre, shows: response.data.results, userId });
  } catch (error) {
    console.error(error);
